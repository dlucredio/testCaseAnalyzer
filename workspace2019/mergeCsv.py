...
def findLine(projectName, testName, iProjectName, iTestName, a):
    for line in a:
        if line[iProjectName] == projectName and line[iTestName] == testName:
            return line
    return None

def countLines(projectName, testName, iProjectName, iTestName, a):
    ret = 0
    for line in a:
        if line[iProjectName] == projectName and line[iTestName] == testName:
            ret += 1
...
parser.add_argument('csv_file_mehran',
                    type=str,
                    help='the second file - Merhan')
...
parser.add_argument('csv_file_unused_mehran',
                    type=str,
                    help='the file containing lines from Mehran\'s file that were never used')
