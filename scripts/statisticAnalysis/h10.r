...
testH10 <- function(locComplexityEntropyCommitsClones) {
  ...
  w = wilcox.test(dataEffective$Cyclomatic, dataIneffective$Cyclomatic, paired = F)
  result$wilcox <- w$statistic
  result$wilcoxPvalue <- w$p.value
  resDelta <- cliff.delta(dataEffective$Cyclomatic, dataIneffective$Cyclomatic, paired = F)
