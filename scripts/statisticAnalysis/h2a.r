...
testH2a <- function(staticAnalysisCommitsClones) {
  ...
  tryCatch(
    expr = {
      hKendall <- cor.test(data$numberOfDistinctMethodInvocations,
                   data$noCommitFixes,
                   method="kendall",
                   exact=FALSE)
  ...
  tryCatch(
    expr = {
      hSpearman <- cor.test(data$numberOfDistinctMethodInvocations,
                            data$noCommitFixes,
                            method="spearman",
                            exact=FALSE)
  ...
  tryCatch(
    expr = {
      hPearson <- cor.test(data$numberOfDistinctMethodInvocations,
                             data$noCommitFixes,
                             method="pearson")
  ...
  w = wilcox.test(dataEffective$numberOfDistinctMethodInvocations, dataIneffective$numberOfDistinctMethodInvocations, paired = F)
  result$wilcox <- w$statistic
  result$wilcoxPvalue <- w$p.value
  resDelta <- cliff.delta(dataEffective$numberOfDistinctMethodInvocations, dataIneffective$numberOfDistinctMethodInvocations, paired = F)
