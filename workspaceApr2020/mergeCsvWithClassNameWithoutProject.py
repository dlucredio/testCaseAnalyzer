...
def findLine(filePath, testName, iFilePath, iTestName, a):
    className = filePath.split("/")[-1]
    for line in a:
        lineClassName = line[iFilePath].split("/")[-1]
        if lineClassName == className and line[iTestName] == testName:
            return line
    return None

def countLines(filePath, testName, iFilePath, iTestName, a):
    className = filePath.split("/")[-1]
    ret = 0
    for line in a:
        lineClassName = line[iFilePath].split("/")[-1]
...
for lineDaniel in dataDaniel:
    ...
    if countLines(filePath, testName, 2, 4, dataDaniel) > 1:
        duplicateTestCasesDaniel.append(lineDaniel)
    else:
        if countLines(filePath[1:], testName, 2, 1, dataAndrew) > 1:
            duplicateTestCasesAndrew.append(lineDaniel)
            lineDaniel.append('no')
        else:
            lineAndrew = findLine(filePath[1:], testName, 2, 1, dataAndrew)
            if lineAndrew != None:
