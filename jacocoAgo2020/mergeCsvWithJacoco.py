...
def main():
    ...
    header = [ 'mainProjectName', # Main project name (GitHub project)
                    ...
                    'INSTRUCTION_missed', # Number of instructions missed (from JaCoCo)
                    'INSTRUCTION_covered', # Number of instructions covered (from JaCoCo)
                    'LINE_missed', # Number of LOC missed (from JaCoCo)
                    'LINE_covered', # Number of LOC covered by this test case (from JaCoCo)
                    'COMPLEXITY_missed', # Complexity of the missed instructions (from JaCoCo)
                    'COMPLEXITY_covered', # Complexity of the covered instructions (from JaCoCo)
                    'METHOD_missed', # Number of methods missed (from JaCoCo)
                    'METHOD_covered', # Number of methods covered (from JaCoCo)
                    'BRANCH_missed', # Number of branches missed (from JaCoCo)
                    'BRANCH_covered' # Number of branches covered (from JaCoCo)
                    ]
...
# JaCoCo's file:
# projectName = 0
# typeName = 1 (ex: hibernate/search/backend/impl/lucene/overrides/ConcurrentMergeScheduler)
# testName = 2
    ret = 0
    typeNameWithSlash = typeName.replace('.','/')
    for line in dataJaCoCo:
        if line[0] == projectName and line[1] == typeNameWithSlash and line[2] == testName and int(line[6]) > 0:
            ret += 1
    return ret
